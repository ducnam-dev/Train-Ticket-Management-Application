...
public List<ChuyenTau> getAllChuyenTau() throws SQLException {
    ...
    public boolean addChuyenTau(ChuyenTau ct) throws SQLException {
        ...
        // Câu SQL dựa trên CSDL QuanLyVeTauTest2
        // ChuyenTau(MaChuyenTau, MaTau, MaNV, GaDi, GaDen, NgayKhoiHanh, GioKhoiHanh, NgayDenDuKien, GioDenDuKien, TrangThai)
        String sql = "INSERT INTO ChuyenTau (MaChuyenTau, MaTau, MaNV, GaDi, GaDen, " +
                "NgayKhoiHanh, GioKhoiHanh, NgayDenDuKien, GioDenDuKien, TrangThai) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        int rowsAffected = 0;

        // --- Kiểm tra dữ liệu đầu vào cơ bản ---
        if (ct == null || ct.getMaChuyenTau() == null || ct.getTau() == null ||
                ct.getGaDi() == null || ct.getGaDen() == null || ct.getNgayKhoiHanh() == null ||
                ct.getNgayDenDuKien() == null) { // NgayDenDuKien là NOT NULL trong CSDL mới

            throw new IllegalArgumentException("Thông tin chuyến tàu không hợp lệ (thiếu mã, tàu, ga hoặc ngày).");
        }
        ...
        try {
            ...
            // CSDL: ChuyenTau.MaTau tham chiếu đến Tau.SoHieu
            // Giả sử entity Tau của bạn có getSoHieu()
            pstmt.setString(2, ct.getTau().getSoHieu());

            // Xử lý MaNV (có thể null)
            if (ct.getNhanVien() != null && ct.getNhanVien().getMaNV() != null) {
                pstmt.setString(3, ct.getNhanVien().getMaNV());
            } else {
                pstmt.setNull(3, Types.NVARCHAR);
            }

            // CSDL: ChuyenTau.GaDi/GaDen lưu TenGa
            // Giả sử entity Ga của bạn có getTenGa()
            pstmt.setString(4, ct.getGaDi().getTenGa());
            pstmt.setString(5, ct.getGaDen().getTenGa());

            // Chuyển đổi java.time.LocalDate/LocalTime sang java.sql.Date/Time
            pstmt.setDate(6, java.sql.Date.valueOf(ct.getNgayKhoiHanh()));
            pstmt.setTime(7, (ct.getGioKhoiHanh() != null) ? java.sql.Time.valueOf(ct.getGioKhoiHanh()) : null);
            ...
            if (e.getMessage().contains("PRIMARY KEY constraint")) {
                throw new SQLException("Mã chuyến tàu '" + ct.getMaChuyenTau() + "' đã tồn tại.", e.getSQLState(), e);
            }
            throw e; // Ném lại lỗi
        } finally {
            // Đóng PreparedStatement
            ...
            // Không đóng Connection ở đây (ConnectDB quản lý)
        }
        return rowsAffected > 0;
    }
